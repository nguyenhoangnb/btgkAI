...
def sentence3() -> Expr:
    ...
    A=PacmanAlive_1%(PacmanAlive_0&~PacmanKilled_0|~PacmanAlive_0&PacmanBorn_0)
    B=~(PacmanAlive_0&PacmanBorn_0)
    return conjoin(A,B,PacmanBorn_0)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
from logic_utils import num_or_str
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    class lowerCase:
        def __init__(self, lower_op):
            self.lower_op = lower_op
        
        def __repr__(self):
            return self.lower_op
    dic=dict()
    dic[lowerCase(a.op.lower())]=True
    return dic
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for i in range(0, noLiterals):
        for j in range(i+1, noLiterals): 
            conjunc.append(disjoin(~literals[i], ~literals[j]))
    
    return conjoin(conjunc)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    for food_coords in food: KB.append(PropSymbolExpr(food_str, food_coords[0], food_coords[1], time = 0))
